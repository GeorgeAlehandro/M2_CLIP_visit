...
model <- vv$loadModel("/data/velosim.json","/data/velosim_weights.h5")
...
velocity <- t(simulated_data$velocity)
groups<-simulated_data$backbone
plot(model[[2]](velocity))
originalPCA_plus_velocity <- tv1$data + 0.01*velocity 
