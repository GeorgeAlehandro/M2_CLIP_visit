...
def plot_palantir_results_f(pr_res, tsne, point_size=3, labels = None, origin = None):
    """ Plot Palantir results on tSNE
    """
    # fix labels (deprecated pandas operation in original code)
    tsne = tsne.loc[tsne.index.intersection(pr_res.pseudotime.index)]
    # Set up figure
    ...
    ax.scatter(tsne.iloc[:, 0], tsne.iloc[:, 1], s=point_size, cmap=matplotlib.cm.plasma, c=c)
    ax.scatter(
        tsne.iloc[origin,0], 
        tsne.iloc[origin,1], 
        s=12, cmap=matplotlib.cm.plasma, c='red'
    )
    ...
    for i, branch in enumerate(pr_res.branch_probs.columns):
        ...
        ax.scatter(
            tsne.iloc[:, 0], tsne.iloc[:, 1], s=point_size, cmap=matplotlib.cm.plasma, c=c
        )
        ax.scatter(
            tsne.loc[pr_res.branch_probs.columns[i],tsne.columns[0]], 
            tsne.loc[pr_res.branch_probs.columns[i],tsne.columns[1]], 
            s=12, cmap=matplotlib.cm.plasma, c='limegreen'
        )
        ax.scatter(
            tsne.iloc[origin,0], 
            tsne.iloc[origin,1], 
        ...
        cbar = matplotlib.colorbar.ColorbarBase(cax, norm=normalize, cmap=cmap)
        ax.set_axis_off()
        ax.set_title(str(branch)+' '+str(labels[np.where(pr_res.pseudotime.index.to_numpy()==branch)[0][0]]), fontsize=10)
