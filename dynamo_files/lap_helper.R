...
build_Laplacian_and_RHS<-function(A,origin,weights=NULL){
  ...
  # Calculating the rowsums of the input graph (INPUT GRAPH IS THE transition matrix)
  .DD <- Matrix::rowSums(A)
  # Creating the degreematrix from the calculated degrees of each node (step just done above)
  DegreeMatrix <- Matrix::Diagonal(x = .DD)
  # To NORMALIZE the Laplacian matrix
...
build_sym_Laplacian_and_RHS<-function(A,origin,weights=NULL){
  ...
  # To build a normalized symmetric Laplacian matrix
  L <- Dm %*% (D - A) %*% Dm
  rm(D)
  unlabeled <- which(!(1:nrow(L) %in% origin))
  L <- L[unlabeled, unlabeled]
